...
class QLearning:
	...
	def learn(self, previous_state, previous_action, previous_reward, new_state):
		q_values = []
		for action in self.actions:
			q_values.append(self.get_q_value(new_state, action))
		...
		self.q[(previous_state, previous_action)] = current_value + self.decay(previous_state, previous_action) * ((previous_reward + self.gamma * max_q) - current_value)